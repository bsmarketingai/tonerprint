...
- **Značka:** logo TonerPrint (čtyři tečky + wordmark) jako **PNG na bílém podkladu** —
  `assets/brand/logo-tonerprint.png`. Jiná varianta neexistuje a nebude; vektor ani
  reverzní verze nejsou k dispozici. **Nikdy logo nepřekresluj do SVG ani nesázej
  wordmark z písma** — vlož tenhle soubor. Na tmavém podkladu (např. `--color-main-900`)
  jde na bílou plotnu se zaoblením `--radius-6`, protoež wordmark je navy.
...
### Vědomá výjimka proti kontrole design systému

Kontrola hlásí **84 custom properties deklarovaných v komponentových selektorech**
(`.button.a2`, `.button.small`, `._filterButton`, `.ProductActionPanelView .button.active`,
`.productFlag.flag01`–`flag08`, …) a navrhuje přesun pod `:root` nebo `[data-*]`.
**Neděláme to a dělat se to nemá.**

Základní hodnoty (`--btn-bg-color`, `--btn-text-color`, `--btn-border-*`) jsou tokeny
v `:root`. Varianta tlačítka se v BS Shopu mění právě tím, že proměnnou předeklaruje
ve svém selektoru — základní pravidlo `.button { background: var(--btn-bg-color) }`
zůstává jedno. Přesun těch přepisů pod `:root` by obarvil všechna tlačítka poslední
variantou; přesun pod `[data-*]` by vyžadoval zásah do HTML, které je dané platformou.

Trik `background: var(--btn-bg-color, fallback)` nefunguje: proměnná v `:root`
existuje, takže se fallback nikdy neuplatní. Totéž platí pro `--bg-color` /
`--text-color` u vlajek.

Co z hlášení opraveno bylo: paleta vlajek je vytažená do `:root` jako
`--flag-01-bg-color` … `--flag-08-text-color` (tokenů 449 → 465), variantní
selektory na ni jen odkazují.

- **Sociální sítě:** Tabler outline `brand-facebook`, `brand-instagram`, `brand-youtube`,
  `brand-linkedin`, `brand-x` — dodané zadavatelem, zařazené do sprite jako `#tp-brand-*`.
  Nejsou to barevná loga značek: kresba je stejná jako u zbytku sady (stroke 2, mřížka
  24 × 24) a barvu dědí přes `currentColor`. V patičce sedí v kolečku 44 × 44 px.
